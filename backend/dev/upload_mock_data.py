    ...
    with QuinoContextManager() as db:
        try:
            ...
            if existing_projects:
                ...
                raise RuntimeError(
                    "Database already contains projects. Use delete_all_data.py to clear the database first."
                )
            ...
            for i, test_case in enumerate(test_cases):
                for j, browser_config in enumerate(browser_configs):
                    traversal = TestTraversalRepo.create(
                        db,
                        CreateTestTraversal.sample_factory_build(test_case.id, browser_config.id),
                    )
            ...
            for i, traversal in enumerate(traversals[:2]):
                for secret_value in secret_values:
                    ...
                    db.add(link)
                    rich_print(f"✓ Linked secret '{secret_value.secret_name}' to traversal {i+1}")
